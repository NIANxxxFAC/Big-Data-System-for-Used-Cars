...
def already_found(index):
    path = "car_data"  # 文件夹目录
    files = os.listdir(path)
    return "cars_{index}.json".format(index=index) in files
...
def get_info_from_list(args):
    ...
    for i, car_id in enumerate(car_id_list):
        ...
        content = soup.html
        try:
            if content.h1.text == 'Access Denied':
                print('Access Denied')
                exit()
        ...
        # car basic info
        basics_json = dict()
        basics = get_content_by_class(content, 'vdp-details-basics__list')
        if basics:
        ...
        try:
            ...
            try:
                ...
                try:
                    key_value = re.search(r".*detail/(\d{7})", url).group(1)
                except AttributeError:
                    continue
